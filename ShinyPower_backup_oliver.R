      ...
      tabItem(tabName = "lmpdata",
        fluidRow(
           box(title="Lay Monitoring Total Phosphorus - All Data", width=6, solidHeader = T, 
              ...
              plotlyOutput("dailyByYear", height="310px") 
           ),
           column(width=4,
...
server <- function(input, output, session) {
  ...
  listPowerLMP <- reactive({
    ...
    list(tab.parametric = tab.parametric, tab.nonparametric = tab.nonparametric,
         ...
         df.iso.nonparametric=df.iso.nonparametric, df.annmean=df.annmean)  
  ...
  output$dailyByYear <- renderPlotly({
    ...
    df <- listDataLMP()$df.allData
    df.annmean <- listDataLMP()$df.annmean
    ...
    # p <- p %>% add_trace(data=df.annmean, x = ~Year, y=fitted(annmean ~ as.numeric(Year), df.annmean), type="scatter", 
    #                      name="Annual Mean", mode='lines', 

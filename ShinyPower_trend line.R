...
ui <- dashboardPage(
  ...
  dashboardSidebar(width=300,
    sidebarMenu(
      menuItem("LMP", tabName = "data", icon = icon("sun"), startExpanded=T, 
      ...
      menuItem("Spring P", tabName = "power", icon = icon("frog"), startExpanded=T,
  ...
  dashboardBody(tags$head
    ...
    tabItems(
      ...
      tabItem(tabName = "lmpdata",
        fluidRow(
           box(title="Lay Monitoring Total Phosphorus", width=6, solidHeader = T, 
              status="primary", height=380,
              plotlyOutput("dailyByYear", height="320px") 
           ),
           column(width=4,
              box(title="Select Y-axis Scale", width=NULL, solidHeader = T, status="success",
                  height=120,
                   radioButtons("lmpyscale", label="", choiceNames=c("Linear", "Logarithmic"), 
        ...
        fluidRow(
          column(width=6,
            box(title="Power - Median greater than TP concentration (non-parametric test)", width=NULL,
                ...
                div(style = "padding: 0px; margin-top: -1em;", (dataTableOutput("powerTableNonParametric"))
                )
            ),
            box(title="Power - Mean greater than TP concentration (parametric test)", width=NULL,
                solidHeader=F, status="success", height=380, 
                div(style = "padding: 0px; margin-top: -1em;", (dataTableOutput("powerTableParametric"))
                )
            )
          ),
          column(width=6,
...
server <- function(input, output, session) {
  ...
  listLMP <- reactive({
    ...
    for (i in c(2:10)) { # i is the number of reps
      for(z in seq(.5,.9, .1)) {
        ...
        df.iso.parametric[nrow(df.iso.parametric)+1,] <- c(yrs, i, z)
      }
    }
    ...
    for (i in c(2:10)) { # i is the number of reps
      for(z in seq(.5,.9, .1)) {
        ...
        yrs <- r * n1 + (1 - r) * ((n1 - 1) / i + 1)
        df.iso.nonparametric[nrow(df.iso.nonparametric)+1,] <- c(yrs, i, z)
      }
    }
    ...
    list(tab.parametric = tab.parametric, tab.nonparametric = tab.nonparametric,
         ...
         df.iso.nonparametric=df.iso.nonparametric, df.allData = df.allData, df.annmean=df.annmean)  
    
    # calculate data for trend line
  ...
  listSprTP <- reactive({
    ...
    # Grab data 
    channel <- odbcDriverConnect('driver={SQL Server};server=anrgdb;database=WQdata;trusted_connection=true')
    df <- sqlQuery(channel, paste0("SELECT LakeID, Year, SprTP FROM 
                    dbo.SprData_AnnualMeans_Sta1 WHERE SprTP is not NULL  
                    AND Year >= ", yearStart, " AND Year <=", yearEnd, " AND LakeID='", myLakeID, "'"))
    odbcClose(channel)
    
    # values needed for power analysis
    alpha <- 0.05 # alpha for p-value
    mu <- input$mu # target phosphorus for effect size
    alltime.mean <- mean(df$SprTP)
    alltime.sd <- sd(df$SprTP)
    d.cohen <- abs(alltime.mean - mu) / alltime.sd
    ...
    m <- data.frame(Years = c(1:10))
    
    # this is power for the parametric t test 
    tab.parametric.SprTP <- m
    tab.parametric.SprTP$Power <- f.power.parametric(tab.parametric.SprTP$Years) 
    
    # this is power for the non parametric sign test based on Noether and the USGS book
    tab.nonparametric.SprTP <- m
    tab.parametric.SprTP$Power <- f.power.parametric(tab.nonparametric.SprTP$Years)  
    
    # data for isotherm plot - parametric test
    # power = 0.8
    df.iso.parametric.SprTP <- data.frame(years=numeric(), power=numeric())
    for(z in seq(.5,.9, .1)) {
      n1 <- pwr.t.test(power=z, d = d.cohen, sig.level = alpha, type = "one.sample", alternative = "greater")$n
      df.iso.parametric.SprTP[nrow(df.iso.parametric.SprTP)+1,] <- c(n1, z)
    }
    
    df.iso.nonparametric.SprTP <- data.frame(years=numeric(), power=numeric())
    for(z in seq(.5,.9, .1)) {
      zalpha <- qnorm(alpha)
      zbeta <- qnorm(1 - z)
      p  <- nrow(df[df$Phos>=mu,]) / nrow(df)
      n1 <- (zalpha + zbeta)^2 / (4 * (p - .5)^2)
      df.iso.nonparametric.SprTP[nrow(df.iso.nonparametric.SprTP)+1,] <- c(yrs, z)
    }

    # test normality of annual means
    
    normal.p.SprTP <- df %>% summarize(shapiro=list(shapiro.test(df$SprTP)$p.value)) %>% 
      unlist() %>% as.numeric()
    
    if (normal.p.SprTP < .05) {normal.SprTP <- FALSE} else {normal.SprTP <- TRUE}
    
    list(tab.parametric.SprTP = tab.parametric.SprTP, tab.nonparametric.SprTP = tab.nonparametric.SprTP,
         normal.p=round(normal.p.SprTP,4), normal.SprTP=normal.SprTP, df.iso.parametric.SprTP=df.iso.parametric.SprTP,
         df.iso.nonparametric.SprTP=df.iso.nonparametric.SprTP) 
    
    # calculate Theil-Sen slope end points
    slope <- TSslope(df.Year, df.annmean)
    intercept <- TSintercept(df.Year, df.annmean)
    ...
    if (listLMP()$normal == F) {
      ...
      tagList(
        box(title="Normality", width=NULL, solidHeader=T, status="danger", height=85,
            #p(style="margin-top: -.5em;", paste("Annual means are not normally distributed Shapiro-Wilkes p-value = ", listLMP()$normal.p))
            HTML(paste0("<div style='margin-top: -.5em;'>", "Annual means are not normally distributed", 
                        "<br/>", "Shapiro-Wilkes p-value = ", listLMP()$normal.p, "</div>"))
        )
      )
    } else {
      tagList(
        box(title="Normality", width=NULL, solidHeader=T, status="primary", height=85,
            p(paste0("Annual means are normally distributed, Shapiro-Wilkes p-value = ", listLMP()$normal.p))
        )
      )
    }
  ...
  # plot LMP daily data by year
  output$dailyByYear <- renderPlotly({
    
    df <- listLMP()$df.allData
    df.annmean <- listLMP()$df.annmean
    
    p <- plot_ly()
    p <- p %>% add_trace(data = df, x = ~Year, y = ~Phos, name="Sample", type= "scatter", mode = "markers", marker = list(size = 10,
                color = '#FFFFFF', line = list(color = 'rgba(75, 46, 57, .8)',  width = 2))) 
    p <- p %>% add_trace(data=df.annmean, x = ~Year, y=~annmean, name="Annual Mean", type="scatter", mode='markers', 
                         marker = list(size = 12, color = '#3C8DBC', line = list(color = 'rgba(75, 46, 57, .8)',  width = 2)))
    p <- p %>% layout(yaxis=list(title="Phos ug/L"), legend=list(orientation='h', xanchor='center',x=.5, y=-.3)) %>%
  ...
  output$powerTableNonParametric <- DT::renderDT({
    ...
    df <- listLMP()$tab.nonparametric
    
    datatable(df, container = sketch, rownames=T, class="compact",
              colnames=F, options = list(dom = "t", ordering=F, 
  ...
  # plot LMP power  parametric
  output$powerParametricPlot <- renderPlotly({
    
    df <- listLMP()$df.iso.parametric
